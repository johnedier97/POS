...
Optimización de consultas SQL en el sistema POS, reemplazando patrones N+1 con consultas batch y agrupadas (`GROUP BY`, `whereIn`, `findMany`), y configurando carga eager de relaciones (`role`, `unitOfMeasure`) para reducir la cantidad de queries a la base de datos.
